...
#define RTP_PAYLOAD_TYPE_PCMA   98
...
typedef struct{
    /* byte 0 */
    uint8_t csrcLen:4;//crc标识符个数
    uint8_t extension:1;//扩展标志,为1时表示rtp报头后有1个扩展包
    uint8_t padding:1;//填充标志,为1时标识数据尾部有无效填充
    uint8_t version:2;//版本号

    /* byte 1 */
    uint8_t payloadType:7;//有小载荷类型
    uint8_t marker:1;//载荷标记,视频为1帧结束,音频为会话开始
